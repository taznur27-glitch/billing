...
export function getOldStockFlag(item: any): boolean {
  ...
  return days > 30; // older than 30 days = old stock
}
export const useAddParty = () => {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: async (newParty: any) => {
      const { data, error } = await supabase
        .from('parties')
        .insert([newParty])
        .select();
...
export const useAddReturn = () => {
  ...
  return useMutation({
    mutationFn: async (newReturn: any) => {
      const { data, error } = await supabase
        .from('returns')
        .insert([newReturn])
        .select();
...
export function getWarrantyAlert(item: any): {
  label: string;
  type: 'valid' | 'expiring';
} {
  if (!item?.warranty_expiry) {
    return { label: 'No Warranty', type: 'valid' };
  }
  ...
  if (diffDays <= 30) {
    return { label: 'Expiring Soon', type: 'expiring' };
  }
